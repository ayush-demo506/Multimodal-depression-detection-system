#!/usr/bin/env python3
"""
Comprehensive Dataset preparation script for multimodal depression detection.
Creates large, detailed datasets with suggestions and analysis capabilities.
"""
...
def load_existing_data():
    ...
    # Load datasets
    depressed_df = pd.read_csv(data_dir / "depressed_2000.csv")
    positive_df = pd.read_csv(data_dir / "positive_2000.csv")
    
    print(f"Loaded {len(depressed_df)} depressed samples")
    print(f"Loaded {len(positive_df)} positive samples")
    
    return depressed_df, positive_df
...
def create_comprehensive_samples(depressed_df, positive_df, num_samples=10000):
    ...
    all_texts = []
    all_texts.extend([(text, 1) for text in depressed_df['text'].tolist()])  # 1 = depressed
    all_texts.extend([(text, 0) for text in positive_df['text'].tolist()])   # 0 = not depressed
    ...
    for i in range(num_samples):
        ...
        if is_depressed:
            available_texts = [text for text, label in all_texts if label == 1]
        else:
            available_texts = [text for text, label in all_texts if label == 0]
...
def main():
    ...
    # Load existing data
    depressed_df, positive_df = load_existing_data()
    
    # Create comprehensive samples
    print(f"\nGenerating comprehensive samples...")
    samples = create_comprehensive_samples(depressed_df, positive_df, num_samples=15000)
